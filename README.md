...
Укажите путь к .json файлам в аргументе к скрипту:
```
python manage.py load_place --path <путь до файла>
```
Так же можете загрузить данные в БД указав url к .json файлу:
```
python manage.py load_place --url <путь до файла>
```
```
{
